(function () {
    ...
    var botCreatorIDs = ["4546580" , "5995635"];
    ...
    var basicBot = {
        ...
        chatLink: "https://raw.githubusercontent.com/italocjs/RadioactiveBot/master/en.json",
        ...
        roomUtilities: {
            rankToNumber: function (rankString) {d
                var rankInt = null;
                switch (rankString) {
                    case "admin":
                        rankInt = 10;
        ...
        startup: function () {
            var u = API.getUser(),
                startupPermission = [4546580, 5995635];
  ...
  baterCommand: {
                ...
                arraybater: [
                            "deu-lhe uma voadora no cu",
                            "deu-lhe um soco no olho",
                            "deu-lhe um dedao no cu",
                            "bateu uma para voce",
                            "deu-lhe uma rasteira na rua",
                            "deu-lhe um beslicao na bunda",
                            "deu-lhe um pontape no cachorro",
                            "bateu em um bebado na rua",
                            "te bateu com um gato morto",
                            "te empurrou ladeira abaixo",
                            "jogou agua na sua cara",
                            "deu-lhe beijo",
                            "deu-lhe uma bitoca",
                            "deu-lhe um socao na costela",
                            "te jurou de morte",
                            "te entregou para a policia",
                            "te chamou de macaco da ilha",
                            "arrancou sua fimose",
                            "te deu uma surra de piroca",
                            "dancou kpop pra voce",
                            "dancou kpop com voce"
                          ],
                sebater: 'Vc e retardado de se bater?',
                naomarcoubater: 'marque alguem imbecil',
                baternaotanasala: 'a pessoa nao esta na sala',
                getBater: function () {
                    var c = Math.floor(Math.random() * this.arraybater.length);
                    return this.arraybater[c];
                },
                functionality: function (chat, cmd) {
                    if (this.type === 'exact' && chat.message.length !== cmd.length) return void (0);
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var space = msg.indexOf(' ');
                        if (space === -1) {
                            API.sendChat(this.naomarcoubater);
                            return false;
                        }
                        else {
                            var name = msg.substring(space + 2);
                            var user = basicBot.userUtilities.lookupUserName(name);
                            if (user === false || !user.inRoom) {
                                return API.sendChat(subChat(this.baternaotanasala, {name: name}));
                            }
                            else if (user.username === chat.un) {
                                return API.sendChat(subChat(this.sebater, {name: name}));
                            }
                            else {
                                return API.sendChat(subChat(basicBot.chat.kissed, {nameto: user.username, namefrom: chat.un, kiss: this.getBater()}));
                            }
                        }
                    }
                }
...
maeCommand: {
                ...
                arraybater: [
                    "Sua mae aquela puta",
                    "so nao sou seu pai pq sua mae n tinha troco pra 10",
                    "com 40 reais eu comia sua mae e ainda me sobrava 40 reais",
                    "sua mae e tao gorda q pra abracar ela no ano novo tem que comecar em setembro",
                    "Tua Mae e tao gorda que usam o umbigo dela pra lavar roupa",
                    "Tua Mae e tao gorda que ela ficou entalada num buraco negro",
                    "Aparentemente todo mundo que discutiu com voce ja fez sexo com a sua mae",
                    "Tua Mae e tao gorda que quando ela vestiu uma camiseta com um grande H um helicoptero veio e pousou nela",
                    "Tua Mae e tao gorda que que quando vai ao cinema senta perto de todo mundo",
                    "Tua Mae e tao gorda, que tira foto 3×4 via satelite",
                    "Tua Mae e tao gorda que tem um CEP só pra ela",
                    "Tua Mae e tao gorda que quando ela sobe na balanca aparece Um por vez por favor",
                    "Tua Mae e tao feia q tem filhos de 3 e 9 anos e todos pensam que sao gemeos pq n acreditam que comeram ela 2 vezes",
                    "Tua Mae e tao fracassada que e a mesma mae que o Karusoru",
                    "Tua mae e o ata ata",
                    "Comi a sua mae"
                          ],
                sebater: 'Vc e retardado de se bater?',
            ...
            rouletteCommand: {
                ...
                functionality: function (chat, cmd) {
                    if (this.type === 'exact' && chat.message.length !== cmd.length) return void (0);
                    if ( chat.uid == '5716786' || basicBot.commands.executable(this.rank, chat)) {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (basicBot.commands.executable(this.rank, chat)) {
                        var id = chat.uid,
                            ...
                            thorCd = false,
                            timeInMinutes = 0,
                            dignoAlgo = (id == 4546580 ? 10 : (Math.floor(Math.random() * 10) + 1)),
